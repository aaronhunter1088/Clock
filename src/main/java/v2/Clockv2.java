...
public class Clockv2 extends JFrame { // implements Runnable {
    ...
    protected Time.Month monthAsTime;
    ...
    public Clockv2(int hours, int minutes, int seconds, Time.Month month, String day, int date, int year, Time.AMPM time) throws ParseException {
        super();
        setResizable(true);
        setLayout(layout);
        constraints = new GridBagConstraints();
    ...
    public Clockv2(int hours, int minutes, int seconds, Date today, Time.AMPM ampm) throws ParseException {
        super();
        setResizable(true);
        setLayout(layout);
        constraints = new GridBagConstraints();
        ...
        cal.setTime(today);
        Time.Month month = convertMonthToStr(cal.get(Calendar.MONTH)+1);
        String day = convertDayToStr(cal.get(Calendar.DAY_OF_WEEK));
        int date = cal.get(Calendar.DAY_OF_MONTH);
        int year = cal.get(Calendar.YEAR);
        setGUI(hours, minutes, seconds, month, day, date, year, ampm);
        setMenuBar();
        setClockFace(ClockFace.StartFace);
        pack();
    }
    ...
    public String getTimeAsStr() {
        return this.hoursAsStr+":"+this.minutesAsStr+":"+this.secondsAsStr+" "+this.ampm.getStrValue();
    }
    ...
    public String getDateAsStr() {
        try {
            return Integer.toString(convertMonthToInt(this.monthAsTime))+"/"+this.date+"/"+this.year;
    ...
    public String getPartialDateAsStr() {
        return this.day.substring(0,3)+" "+this.month.substring(0,3)+" "+Integer.toString(this.date)+", "+this.year;
    }
    
    public ClockFace getClockFace() { return this.clockFace; }

    public void setGUI(int hours, int minutes, int seconds, Time.Month month, String day, int  date, int year, Time.AMPM time) throws ParseException {
    ...
    public void setGUI() throws ParseException {
        ...
        monthAsTime = convertMonthToStr(calendar.get(Calendar.MONTH)+1);
        this.month = monthAsTime.strValue;
...
//                this.hours += 1;
        ...
        switch (monthAsTime) {
            case JANUARY: {
                if (this.date == 31 && dateChanged) {
                    this.date = 1;
                    this.month = Time.Month.FEBRUARY.strValue;
                    monthAsTime = Time.Month.FEBRUARY;
                }
                break;
            }
            case FEBRUARY: {
                if ((this.date == 28 || this.date == 30) && this.dateChanged) {
                    this.date = 1;
                    this.month = Time.Month.MARCH.strValue;
                    monthAsTime = Time.Month.MARCH;
            ...
            case MARCH: {
                if (this.date == 32 && this.dateChanged) {
                    this.date = 1;
                    this.month = Time.Month.APRIL.strValue;
                    monthAsTime = Time.Month.APRIL;
                }
                break;
            }
            case APRIL: {
                if (this.date == 31 && this.dateChanged) {
                    this.date = 1;
                    this.month = Time.Month.MAY.strValue;
                    monthAsTime = Time.Month.MAY;
                }
                break;
            }
            case MAY: {
                if (this.date == 32 && this.dateChanged) {
                    this.date= 1;
                    this.month = Time.Month.JUNE.strValue;
                    monthAsTime = Time.Month.JUNE;
                }
                break;
            }
            case JUNE: {
                if (this.date == 31 && this.dateChanged) {
                    this.date = 1;
                    this.month = Time.Month.JULY.strValue;
                    monthAsTime = Time.Month.JULY;
                }
                break;
            }
            case JULY: {
                if (this.date == 32 && this.dateChanged) {
                    this.date = 1;
                    this.month = Time.Month.AUGUST.strValue;
                    monthAsTime = Time.Month.AUGUST;
                }
                break;
            }
            case AUGUST: {
                if (this.date == 32 && this.dateChanged) {
                    this.date = 1;
                    this.month = Time.Month.SEPTEMBER.strValue;
                    monthAsTime = Time.Month.SEPTEMBER;
                }
                break;
            }
            case SEPTEMBER: {
                if (this.date == 31 && this.dateChanged) {
                    this.date = 1;
                    this.month = Time.Month.OCTOBER.strValue;
                    monthAsTime = Time.Month.OCTOBER;
                }
                break;
            }
            case OCTOBER: {
                if (this.date == 32 && this.dateChanged) {
                    this.date = 1;
                    this.month = Time.Month.NOVEMBER.strValue;
                    monthAsTime = Time.Month.NOVEMBER;
                }
                break;
            }
            case NOVEMBER: {
                if (this.date == 31 && this.dateChanged) {
                    this.date = 1;
                    this.month = Time.Month.DECEMBER.strValue;
                    monthAsTime = Time.Month.DECEMBER;
                }
                break;
            }
            case DECEMBER: {
                if (this.date == 32 && this.dateChanged) {
                    this.date = 1;
                    this.month = Time.Month.JANUARY.strValue;
                    monthAsTime = Time.Month.JANUARY;
                    this.year += 1;
                }
                break;
            }
        ...
        if (this.dateChanged) {
            ...
            int month = 0;
            try {
                month = convertMonthToInt(this.monthAsTime);
            } catch (InvalidInputException iie) {
                System.out.println(iie.getMessage());
            }
            int date = this.date;
    ...
    public String convertDayToStr(int thisDay) {
        switch(thisDay) {
            case 1: return Time.Day.SUNDAY.strValue;
            case 2: return Time.Day.MONDAY.strValue;
            case 3: return Time.Day.TUESDAY.strValue;
            case 4: return Time.Day.WEDNESDAY.strValue;
            case 5: return Time.Day.THURSDAY.strValue;
            case 6: return Time.Day.FRIDAY.strValue;
            case 7: return Time.Day.SATURDAY.strValue;
            default: return Time.Day.UNKNOWN_DAY.strValue;
        }
    }
    
    public int convertMonthToInt(Time.Month monthAsTime) throws InvalidInputException {
        switch (monthAsTime) {
